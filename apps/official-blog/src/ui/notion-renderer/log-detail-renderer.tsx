...
import type { fetchSeriesItem } from '@/lib/fetchers';
...
export const LogDetailRenderer = ({
  ...
  initialData: Awaited<ReturnType<typeof fetchSeriesItem>>;
}) => {
  ...
  const pageProperties = useMemo(
    () =>
      page?.properties as PageObjectResponse['properties'] & {
        ...
        subTitle: CustomRichTextProperty;
      },
    [page]
  );
  ...
  const pageSubTitle = useMemo(
    () => pageProperties.subTitle?.rich_text?.at(0)?.plain_text ?? '',
    [pageProperties]
  );
  ...
  return (
    <>
      <PageLayout title={pageTitle?.at(0)?.plain_text} subTitle={pageSubTitle}>
